...
def plot_inlet_bc_velocity(model, data_bundle: DataBundle, device, scfg):
    ...
    dm_norm, y_pred_norm = _predict_one_local(d_raw, model, device, x_scaler, y_scaler, scfg.amp)
...
def main():
    ...
    # --- Inlet BC diagnostic ---
    try:
        plot_inlet_bc_velocity(model, data_bundle, device, scfg)
    except Exception as exc:
